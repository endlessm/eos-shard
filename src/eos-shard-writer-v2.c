...
static void
cpool_dispose (struct cpool *cpool)
{
...
static uint64_t
cpool_add (struct cpool *cpool, const char *S)
{
...
write_blob (EosShardWriterV2 *self, int fd, struct eos_shard_writer_v2_blob_entry *blob)
{
  ...
  sblob.name_offs = cpool_add (&self->cpool, blob->name);
  sblob.content_type_offs = cpool_add (&self->cpool, blob->content_type);
