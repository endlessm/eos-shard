                                       ...
                                       GError **error)
{
   ...
   * the last added key.
   */
  if (self->n_entries_added > 0)
    {
      size_t key_size = max (CSTRING_SIZE (key), CSTRING_SIZE (self->last_key));
      if (memcmp (key, self->last_key, key_size) < 0)
        {
          g_set_error (error, EOS_SHARD_ERROR, EOS_SHARD_ERROR_DICTIONARY_WRITER_ENTRIES_OUT_OF_ORDER,
...
eos_shard_dictionary_writer_finish (EosShardDictionaryWriter *self, GError **error)
{
  ...
  if (self->n_entries_added != self->n_entries_total)
    {
      g_set_error(error, EOS_SHARD_ERROR, EOS_SHARD_ERROR_DICTIONARY_WRITER_WRONG_NUMBER_ENTRIES,
